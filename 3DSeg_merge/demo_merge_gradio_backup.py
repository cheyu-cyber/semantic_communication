...
DEFAULT_BACKGROUND_CLASSES = "0,1,3,4,5,8,9,10"
...
def _build_point_cloud(depth_map: np.ndarray, image: np.ndarray, stride: int = 4) -> go.Figure:
    height, width = depth_map.shape
    ys = np.arange(0, height, stride)
    xs = np.arange(0, width, stride)
    grid_x, grid_y = np.meshgrid(xs, ys)
    sampled_depth = depth_map[ys[:, None], xs]
    colors_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)[ys[:, None], xs]
    colors_hex = ["#%02x%02x%02x" % tuple(pixel) for pixel in colors_rgb.reshape(-1, 3)]
...
def _run_pipeline(left_path: str, right_path: str, fp16: bool) -> Tuple[np.ndarray, np.ndarray, list[dict], go.Figure]:
    ...
    point_cloud_fig = _build_point_cloud(depth_map, original_bgr)
